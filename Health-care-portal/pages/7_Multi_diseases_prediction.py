...
diabetes_model = pickle.load(open('Health-care-portal/models/models_diabetes_model.sav', 'rb'))
heart_disease_model = pickle.load(open('Health-care-portal/models/models_heart_disease_model.sav', 'rb'))
parkinsons_model = pickle.load(open('Health-care-portal/models/models_parkinsons_model.sav', 'rb'))
...
diabetes_example_names = ["Higher BMI", "Lower glucose", "obesity", "Healthy BMI", "High blood pressure", "Normal glucose", "High insulin level"]
...
heart_example_names = ["Healthy Person","Non Healthy Person"]
...
parkinsons_examples_names=["Non Healthy Person","Healthy Person"]
...
#     #                                 ["Manual Input"] + [diabetes_example_names])
...
#     if diab_use_example != "Manual Input":
#     # Find the index of the selected example name
#         example_idx = diabetes_example_names.index(diab_use_example)
#         diab_defaults = diabetes_examples[example_idx]
#     else:
#         diab_defaults = [""] * 8  # Empty defaults for manual input
...
if selected == 'Diabetes Prediction':
    ...
    # Dropdown to select example or input manually
    diab_use_example = st.selectbox("Choose an example or input manually:", 
                                 ["Choose Examples"] + diabetes_example_names)

    if diab_use_example != "Choose Examples":
        # Find the index of the selected example name
        example_idx = diabetes_example_names.index(diab_use_example)
        diab_defaults = diabetes_examples[example_idx]
    else:
    ...
    if st.button('Diabetes Test Result'):
        ...
        # If any field is empty, show a message asking to fill the data
        if "" in user_input:
            st.error("Please enter all the data before making a prediction.")
        else:
...
#                                      ["Manual Input"] + [heart_example_names])
...
if selected == 'Heart Disease Prediction':
    ...
    heart_use_example = st.selectbox("Choose an example or input manually:", 
                                     ["Choose Examples"] + heart_example_names)

    if heart_use_example != "Choose Examples":
        example_idx = heart_example_names.index(heart_use_example)
        heart_defaults = heart_examples[example_idx]
    else:
        heart_defaults = [""] * 13
    ...
    with col2:
        sex = st.selectbox('Sex', options=['Male', 'Female'], index=0 if heart_defaults[1] == 1 else 1)
    ...
    with col3:
        ca = st.text_input('Major vessels colored by fluoroscopy', value=str(heart_defaults[11]))
    ...
    if st.button('Heart Disease Test Result'):
        ...
        if "" in user_input:
            ...
            # Convert all inputs to float
            user_input = [ 1.0 if x.lower() == "male" else 0.0 if x.lower() == "female" else float(x) for x in user_input]
...
if selected == "Parkinsons Prediction":
    ...
    park_use_example = st.selectbox("Choose an example or input manually:", 
                                    ["Choose Examples"] + parkinsons_examples_names)

    if park_use_example != "Choose Examples":
        example_idx = parkinsons_examples_names.index(park_use_example)
    ...
    with col2:
        sex = st.selectbox('Sex', options=['Male', 'Female'], index=0 if park_defaults[1] == 1 else 1)
    ...
    if st.button("Parkinson's Test Result"):
        ...
        # Checking if any input fields are empty
        user_input = [fo, sex, flo, Jitter_percent, Jitter_Abs,
